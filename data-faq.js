...
const FAQ = [
  ...
  {q:"How do I scan food?", a:"Use <b>Scan barcode</b> for a packaged food or <b>Scan nutrition label</b> for a Nutrition Facts panel. Barcode results check foods you have already saved before using an online food database. Fresh online results should be compared with the package. Label scans fill an editable review form. <b>Nothing is logged until you review it and tap Add to log.</b>"},

  {q:"What if scanned nutrition is wrong or missing?", a:"For an online barcode result, use <b>Correct barcode data</b> and enter the values from the package. BlackPyre saves your correction on this device and uses it first on later scans of the same barcode. If a barcode is not found, enter the label manually and save it if you want to reuse it. The package label is the best source for packaged-food nutrition."},
  ...
  {q:"How do I change, undo, or view a food entry?", a:"After choosing a food, adjust the amount with the <b>slider or amount field</b>. Calories and macros recalculate as the amount changes. Manual foods and scanned foods use the same review controls, and you can also enter a food manually without scanning. Foods without a known gram or liquid measurement can still scale by servings. Nothing is logged until you tap <b>Add to log</b>. After adding a food, BlackPyre keeps your place and shows <b>Undo</b> and <b>View entry</b>. Undo removes that exact new entry. View entry moves to it only when you ask. To correct an older entry, tap its edit control and update the values."},


  {sec:"Training"},

  {q:"How do I log a workout?", a:"Open <b>Train</b>, choose the session, and enter the work you actually perform. Tap <b>Save Exercise</b> as you finish each exercise. Saved exercises become part of the protected workout draft. When the workout is complete, tap <b>Log session</b>. If the app closes first, BlackPyre can offer the saved draft to resume."},
  ...
  {q:"How does automatic progression work?", a:"Automatic progression is optional in <b>Settings → Training</b>. When enabled, BlackPyre looks at the last completed exercise and the programmed target. If the required work was completed consistently, it can preload a small progression for the next session. It never changes a workout that has already been logged."},
  ...
  {q:"Where is my data stored? Is it private?", a:"Your settings, logs, program, saved foods, and recovery data are stored on this device. BlackPyre has no user account or central BlackPyre database that can read your personal log. Online searches send the search or barcode request to the online food service, and optional AI features send only the information you choose when you use them."},
