...
check("v62 suggestion catalog remains precached in the current service worker", (()=>{ const x=fs.readFileSync(path.join(__dirname,"..","sw.js"),"utf8"); return x.includes('"./data-suggestions.js"') && x.includes('const CACHE = "blackpyre-v112-water-card-1"'); })());
...
check("SW cache key matches the BlackPyre web v82 release",
  /const CACHE = "blackpyre-v112-water-card-1";/.test(sw));
check("Phase 1 service-worker cache remains refreshed", sw.includes('const CACHE = "blackpyre-v112-water-card-1"') && !sw.includes('blackpyre-phase1-nutrition-safety-1'));
