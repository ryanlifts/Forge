...
check("static UI and FAQ carry 13–100 bounds, assertive errors, youth/pregnancy scope, and estimate wording", /id="cAge"[^>]*min="13"[^>]*max="100"/.test(html) && /id="calcError"[^>]*role="alert"[^>]*aria-live="assertive"/.test(html) && /id="calcOut"[^>]*aria-live="polite"/.test(html) && /pregnan(?:cy|t)|breastfeeding/i.test(html) && /Estimated metabolism from your logs/.test(html) && /What is Estimated metabolism from your logs/.test(faq) && /safety floor does not mean|floor does not mean/i.test(html+faq));
check("FAQ and refreshed cache cover the youth delta without obsolete calculation assets", /Can teenagers use the calorie and macro calculator/.test(faq) && /complete calculator access/.test(faq) && /20% protein \/ 55% carbohydrate \/ 25% fat/.test(faq) && /whole day—not workouts alone/.test(faq) && /Moderate choice maps conservatively/.test(faq) && /For adults/.test(faq) && /parent or guardian/.test(faq) && !/3,?273|2,?773/.test(html+faq+fs.readFileSync("scripts/01-storage.js","utf8")+fs.readFileSync("scripts/06-settings.js","utf8")) && /blackpyre-v85-no-label-scanner-1/.test(sw) && !/blackpyre-v78-native-parity-7|blackpyre-phase1-nutrition-safety-[12]/.test(sw));
