...
async function runSearch(){
  ...
  if (isOffline()){
    ...
    renderResults(dedupeCatalogFoods([...myHits,...cachedCatalog.foods,...bundledHits,...localHits],20));
  ...
  if(catalog.state!=="ok"||!catalog.foods.length){
    try { offHits=await searchOFF(q); } catch(e) {}
  }
  const combined=dedupeCatalogFoods([...myHits,...catalog.foods,...bundledHits,...localHits,...offHits],30);
  renderResults(combined);
  if(catalog.state!=="ok"&&!offHits.length){
    errEl.textContent="The downloadable food catalog is temporarily unavailable — showing saved and bundled USDA matches. Manual label entry is always available.";
    errEl.classList.remove("hidden");
  } else if(!combined.length){
    errEl.textContent="No reliable match was found. Enter the package label manually and BlackPyre will remember it.";
    errEl.classList.remove("hidden");
  }
  searchBtn.disabled = false; searchBtn.textContent = "Search food database";
}
...
async function runBarcode(){
  ...
  try {
    // The downloadable USDA catalog is primary. It works without an API key
    // and native/web runtimes reuse previously downloaded shards offline.
    ...
    if (isOffline()){
      openCustomForm(code);
      errEl.textContent = "Offline — this barcode is not in a cached catalog shard. Add the label details manually, or reconnect and try again.";
      errEl.classList.remove("hidden");
      return;
    }

    // Open Food Facts remains a separately labeled fallback when USDA has no
    // matching UPC. Its data is never merged into the USDA catalog.
    ...
    if (result.state==="unavailable"){
      ...
      openCustomForm(code);
      errEl.textContent = "That barcode was not found in the USDA catalog or Open Food Facts. Enter the package label below and BlackPyre will remember it on this device.";
      errEl.classList.remove("hidden");
    } else {
...
function syncBarcodeCorrectionReview(food){
  ...
  const eligible=!!(
    food
    &&["Open Food Facts","USDA FoodData Central"].includes(food.sourceLabel)
    &&barcode
  );
  ...
  if (nutrition) nutrition.textContent=scannedValues
    ...
    : "";
  if (source) source.textContent=food ? "SOURCE: "+String(food.sourceLabel||"").toUpperCase() : "";
